	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if(cookies == null){
			...
			for (Cookie cookie : cookies) {
				...
				}//CTOPUP_DASHBOARDS/src/main/webapp/JSP/login.jsp
			}
		}
		if(!auth && !isLoginRequest(hReq)){
			RequestDispatcher dispatcher = request.getRequestDispatcher("/homelogin");
			dispatcher.forward(request, response);
		}
		/*else if(!auth && !isLoginRequest(hReq)){
